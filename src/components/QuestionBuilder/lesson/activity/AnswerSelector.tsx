        ...
        <FormField
            ...
            render={({ field }) => (
                <FormItem className="flex flex-col gap-2">
                    ...
                    <ToggleGroup type={isMultiple ? "multiple" : "single"} variant="outline" onValueChange={field.onChange} className={"justify-start"}>
                        {options.map((option, index) => {
                            return (
                                <ToggleGroupItem value={optionIds[index]} aria-label="Toggle bold" >
                                    <div className={`font-black text-lg rounded-full border w-7 justify-center flex  dark:text-slate-800 text-white ${getBgColor(activityType, optionIds[index], answer)}`}>
                                        {optionIds[index]}
                                    </div>
                                </ToggleGroupItem>
                            )

...
export const FillInTheBlankAnswer: FC<FillInTheBlankQuestionProps> = ({
  ...
  const { status, options, segments, removeTokenFromAnswer, addTokenToAnswer } = useFillInTheBlankAnswerStates(
    question,
    language,
    setLessonState,
  )
  ...
  const handleRemoveToken = useCallback(
    (segment: OptionWithTokens) => !isActionDisabled && removeTokenFromAnswer(segment),
    [removeTokenFromAnswer],
  )

  const handleAddToken = useCallback(
    (option: OptionWithTokens) => !isActionDisabled && addTokenToAnswer(option),
    [addTokenToAnswer],
  )

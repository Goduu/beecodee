...
export const TokenGroupChip: FC<TokenGroupChipProps> = ({ id, optionWithToken, onClick, className = "", isOneLined }) => {
    ...
    return (
        <>
            {optionWithToken.tokens.map((token, index) => {
                return (
                    renderTokenContent(index, token, optionWithToken.status || "neutral", onClick, isOneLined)
                )
            }
            )}
        </>
...
const renderGapContent = (id: string | number, token: Token) => {
    ...
    return (
        <span key={id} className={`text-opacity-0 text-slate-50 ${classNames}`}>
...
const renderTokenContent = (id: string | number, token: Token, status?: AnswerStatus, onClick?: () => void, isOneLined?: boolean) => {
    ...
    switch (token.content) {
        ...
        case '\t':
            return <span key={id} className={`w-14 text-center text-opacity-10 text-slate-50 ${classNames}`} onClick={onClick}>\t</span>
        default:
            return token.type === 'gap' ? renderGapContent(id, token) : (
